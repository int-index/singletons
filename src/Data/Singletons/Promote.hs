...
-- that has a CUSK even when we want kind inference to work. There seems to
-- be no way to avoid this, so we embrace it:
--   * If a class type variable has no explicit kind, we make no effort to guess it and default to *.
--     This is OK because before TypeInType we were limited by KProxy anyway.
--
